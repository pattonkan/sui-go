...
// TODO there should be a better way to replace this
type TaggedJSON interface {
	Tag() string
	Content() string
}

// Polymorphic JSON Handling
type WrapperTaggedJson[T TaggedJSON] struct {
	Data T
}

func (t *WrapperTaggedJson[T]) UnmarshalJSON(data []byte) error {
	if len(data) <= 0 {
